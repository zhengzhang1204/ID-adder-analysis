...
%2 judge if this spot line has a sister, which resides in the same cell and spans almost the same frame range.
    ...
    if isempty(tes)
        [hasSis,curCat,curLrt,sisIdx,errTxt]=ImgFLN_simpleIdentSisChrVasyn(SptLines,iSL,CIDinParts,uit);% Method 1: Use existing map to identify sister.
...
%HASSIS-2 if not all the daughter spot lines are overlapping with mother spot line (i.e., current spot line), then find them in the remaining part of this PH cycle.
        if isnan(desRowLow) || isnan(desRowUp)
            ...
            %if the current PHline has no descendant
            if isnan(HorLineMap(curSLendPHIdx,2)) && isnan(HorLineMap(curSLendPHIdx,3))
                ifCurPHLineIsEnd=true;
            else
                ifCurPHLineIsEnd=false;
            end
            ...
            if ~isempty(cand)
                ...
                Lrts=cellfun(@(x) mean(x,'omitnan'),Lrts);
                if curCat==1
                    if ifCurPHLineIsEnd && length(Lrts)==2
                        selCol=[1,2];
                    else
                        selCol=find(Lrts<curSLLrt+0.25);
                    end
...
%NOSIS-2 if not all the daughter spot lines are overlapping with mother spot line (i.e., current spot line), then find them in the remaining part of this PH cycle.
        if isnan(desRowLow) || isnan(desRowUp)
            ...
            if ~isempty(vSisIdx) && ~isnan(vSisIdx(1,1)) && size(vSisIdx,1)==1
                vSisIdx=uit(vSisIdx,3-find(uit(vSisIdx,:)==iSL));cand(cand==vSisIdx)=[];vSisIdx=unique(vSisIdx);
                if ~isnan(vSisIdx)
                    %see if the vSisIdx starts from a PHpart that is the heir of the current ending CID PHpart.
                    uitSisStPHpartIdx=find(cellfun(@(x) ismember(SptLines{vSisIdx}(1,2:3),x,'rows'),CIDinParts));
                    curSLEndPHpartIdx=find(cellfun(@(x) ismember(SptLines{iSL}(end,2:3),x,'rows'),CIDinParts));
                    if SptLines{vSisIdx}(1,2)>SptLines{iSL}(end,2) && ismember(uitSisStPHpartIdx,HorLineMap(curSLEndPHpartIdx,2:3))%if the starting frame of sister SL does not overlap with curent SL, means less-dnaA-type
                         %if the CID where current SL ends has only this SL:
                         foundSLIdx=find(cellfun(@(x) ~isempty(find(x(:,2)==SptLines{iSL}(end,2) & x(:,3)==SptLines{iSL}(end,3), 1)),SptLines));
                         if length(foundSLIdx)==1%This SL is the only replicating SL in this CID. It is placed at cell center, but its actual Lrt should be 1/4 or 3/4.
                             if ~isempty(find(uit(:,1)==iSL, 1))%current SL should be the upper SL
...
%NOSIS-3 if one or both daughter spot lines are in the daughter cells:
        ...
        if isempty(iRow)
            ...
            if iCol==2%meaning the current sl is the lower one of the mother
                if isnan(uit(iRow,1))%see if its sister (which is the upper daughter of the mother) exists:
                    %(occupies entire) uit sl-sister is nan and current sline occupies the entire cell, look for descendant sl lines in upper and lower daughter PHparts
                    lookInPHidx=HorLineMap(curSLendPHIdx,2:3);
                    
                else%if exists, the uit sl-sister starts from either the daughter PHpart or not. if yes, the current sl occupies half of the cell. if no, the current sl occupies entire cell.
                    
                    daugPHparts=HorLineMap(curSLendPHIdx,2:3);%daughter PHparts:
                    if ~isnan(daugPHparts(1))%see if uit sl-sister starts in the upper daugher cell
                        if ~isempty(find(ismember(SptLines{uit(iRow,1)}(1,2:3),CIDinParts{daugPHparts(1)},'rows'),1))%the uit sl-sister starts is the upper daugher cell
                            %(occupies lower half) look for descendant sl lines in the lower daughter PHpart
                            lookInPHidx=HorLineMap(curSLendPHIdx,3);
                        else
                            %(occupies entire) (may not exist this case) the current sl occupies the entire cell, look for descendant sl lines in upper and lower daughter PHparts
                            lookInPHidx=HorLineMap(curSLendPHIdx,2:3);
                        end
                    end
                    if ~isnan(daugPHparts(2))%see if uit sl-sister starts in the lower daugher cell
                        if ~isempty(find(ismember(SptLines{uit(iRow,1)}(1,2:3),CIDinParts{daugPHparts(2)},'rows'),1))%the uit sl-sister starts is the lower daugher cell
                            %(occupies lower half) look for descendant sl lines in the lower daughter PHpart
                            lookInPHidx=HorLineMap(curSLendPHIdx,3);
                        else
                            %(occupies entire) (may not exist this case) the current sl occupies the entire cell, look for descendant sl lines in upper and lower daughter PHparts
...
%                     lookInPHidx=HorLineMap(curSLendPHIdx,3);%lower descendant index
            ...
            elseif iCol==1%meaning the current sl is the upper one of the mother
                if isnan(uit(iRow,2))%see if its sister (which is the lower daughter of the mother) exists:
                    %(occupies entire) if does not exist, the current sl occupies the entire cell
                    lookInPHidx=HorLineMap(curSLendPHIdx,2:3);
                else%if exists, the uit sl-sister starts from either the daughter PHpart or not. if yes, the current sl occupies half of the cell. if no, the current sl occupies entire cell.
                    daugPHparts=HorLineMap(curSLendPHIdx,2:3);%daughter PHparts:
                    if ~isnan(daugPHparts(1))%see if uit sl-sister starts in the upper daugher cell
                        if ~isempty(find(ismember(SptLines{uit(iRow,2)}(1,2:3),CIDinParts{daugPHparts(1)},'rows'),1))%the uit sl-sister starts is the upper daugher cell
                            %(occupies upper half) look for descendant sl lines in the upper daughter PHpart
                            lookInPHidx=HorLineMap(curSLendPHIdx,2);
                        else
                            %(occupies entire) (may not exist this case) the current sl occupies the entire cell, look for descendant sl lines in upper and lower daughter PHparts
                            lookInPHidx=HorLineMap(curSLendPHIdx,2:3);
                        end
                    ...
                    if ~isnan(daugPHparts(2))%see if uit sl-sister starts in the lower daugher cell
                        if ~isempty(find(ismember(SptLines{uit(iRow,2)}(1,2:3),CIDinParts{daugPHparts(2)},'rows'),1))%the uit sl-sister starts is the lower daugher cell
                            %(occupies upper half) look for descendant sl lines in the upper daughter PHpart
                            lookInPHidx=HorLineMap(curSLendPHIdx,2);
                        else
                            %(occupies entire) (may not exist this case) the current sl occupies the entire cell, look for descendant sl lines in upper and lower daughter PHparts
            ...
            if length(lookInPHidx)==2%two available cells (daughter PHparts) to look for daughter sls.
                if isnan(desRowLow) && ~isnan(lookInPHidx(2))%if the lower sl is missing, look for it in the lower daughter PHpart.
                    PHdesCID=mat2cell(CIDinParts{lookInPHidx(2),1},ones(1,size(CIDinParts{lookInPHidx(2),1},1)));
                    out=cell2mat(cellfun(@(x) find(x(1)==startCID(:,1) & x(2)==startCID(:,2)),PHdesCID,'UniformOutput',false));
                    if ~isempty(out)
                        desRowLow=out(1);
                    end
                end
                if isnan(desRowUp) && ~isnan(lookInPHidx(1))%if the upper sl is missing, look for it in the upper daughter PHpart.
                    PHdesCID=mat2cell(CIDinParts{lookInPHidx(1),1},ones(1,size(CIDinParts{lookInPHidx(1),1},1)));
                    out=cell2mat(cellfun(@(x) find(x(1)==startCID(:,1) & x(2)==startCID(:,2)),PHdesCID,'UniformOutput',false));
                    if ~isempty(out)
                        desRowUp=out(1);
                    end
                end
            elseif length(lookInPHidx)==1%one available cells (daughter PHparts) to look for daughter sls.
                if ~isnan(lookInPHidx(1))
                    PHdesCID=mat2cell(CIDinParts{lookInPHidx,1},ones(1,size(CIDinParts{lookInPHidx,1},1)));
                    out=cell2mat(cellfun(@(x) find(x(1)==startCID(:,1) & x(2)==startCID(:,2)),PHdesCID,'UniformOutput',false));
                    if ismember(uit(iRow,3-iCol),out)
                        out(out==uit(iRow,3-iCol))=[];
                    end
                    for i=1:size(out,1)
