...
EXP='F:\20230521_WT_M6_seqAmChrProc(quick check GR)';
MappingMethod=4;%[4] fast growth
...
for m=4:length(F)%for each pos.<----------------------------------------------change back to 1
    posStr=F(m).name(1:3);
    ...
    for iSchN=16:nSchN%for each side channel<----------------------------------change back to 1
...
%         saveas(fig,'E:\DLstg32DivTree.fig');
    ...
    %5. Manually correct map on the annotated image.    
        ...
        load('D:\Program\Unfinished\stills\CrossPatternFLN.mat');
