function [hasSis,curCat,curLrt,sisIdx,errTxt]=ImgFLN_simpleIdentSisChrVasyn(SLines,iSL,PHLines,map)
...
if ~isempty(iRow)
    ...
    %if the sister is overlapping with the current SLine
    ...
    %idx of PH line containing the head of sister SLine
    idxH=find(cellfun(@(x) ~isempty(find(x(:,1)==SLines{sisIdx,1}(1,2) & x(:,2)==SLines{sisIdx,1}(1,3),1)), PHLines));
