classdef imgABv2 < handle  
    ...
    methods
        ...
        function tool = imgABv2(I,nLabels,RawMsk,TxtIn,minArea,minLength,minY,cutEndY,varargin)
            tool.LineageToSave={};
            tool.DidAnnotate = 0;
            tool.LabelIndex = 1;
            tool.Image = I;
            ...
            uicontrol(tool.Dialog,'Position',[10,271, 100,12],'Style','text','HorizontalAlignment','left','String','v1.3 ZZ@220519','FontSize',7,'ForegroundColor',[0.5,0.5,0.5]);

            % TEST button:
            uicontrol('Parent',tool.Dialog,'Style','pushbutton','String','TEST [t]','Position',[160 210 130 40],'Callback',@tool.btnTESTPush);
            
            % Deterministic controls:
            uicontrol('Parent',tool.Dialog,'Style','pushbutton','String','Emptize','Position',[160 120 130 20],'Callback',@tool.btnEmptizePush);
            tool.DelAfter=uicontrol('Parent',tool.Dialog,'Style','edit','String','1','Position',[230 180 60 20]);
            uicontrol('Parent',tool.Dialog,'Style','pushbutton','String','DelAfter','Position',[160 180 60 20],'Callback',@tool.btnDelAfter);
