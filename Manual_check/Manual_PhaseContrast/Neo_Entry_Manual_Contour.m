clc;
clear;
%This is for manually correct masks
% Version 2.1 @23.02.20 PHfig and everything in lkgM are saved using this codes now.

%% user-specified region
EXP='F:\20230528_WT_M18_DnaNyPetProc';%E:\M25_WT_mCherry';
cutEndY=0;%<-set this back to 0, can be 230.
...
for m=2:length(F)%for each pos.<----------------------------------------------change back to 1
    load(fullfile(F(m).folder,F(m).name),'PosMask','uitCIx','uitPH');%uitCIx=curCI;%'uitCIx'
    if exist(fullfile(F(m).folder,[F(m).name(1:3),'_lkgM.mat']),"file")
    ...
    PosMaskN=PosMask;
    for k=13:nTile%for each side channel<--------------------------------------change back to 1
        msk=[];
        sw=true;
        sw2=true;
        eptcnt=0;
%         out2=genTilizedPH(F(m).folder,F(m).name,k,length(PosMask),uitCIx);   

        out2=zeros(256,32*size(uitPH,3));
        for iFrm=1:size(uitPH,3)
            out2(:,(iFrm-1)*32+1:iFrm*32)=uitPH(:,(k-1)*32+1:k*32,iFrm);
        end
    
%         out2=cell2mat(cellfun(@(x) x{k}, PosMask,'UniformOutput',false));
        
        %check if the first frame is empty. If yes, omit this side channel
        F1=PosMask{1,1}{k,1};
        if cutEndY~=0
            newF1=F1;
        ...
        Msk=imgABv2(mat2gray(out2),1,msk,{tex,xt,yt,Label,Pos(2:3),num2str(k,'%0.2i')},minArea,majorL,minY,cutEndY);
