...
procFrm=[];
modFrames=[];
for iL=1:length(L)%for each dif region, actually, for each frame that contains dif region.
    frame=floor(L(iL).Centroid(1)/32)+1;
    prevTile=prevMsk(:,(frame-1)*32+1:frame*32);
    newTile=Msk(:,(frame-1)*32+1:frame*32);
    %core functions
