...
function [slope,slope2,N,rho]=drawFigDensRescaleV2_simple(edgesX,X0,Y0,nSD,svPath,lgdLabel,med,lab,thrPct)
% Rescaling
xM=mean(X0,'omitnan');
yM=mean(Y0,'omitnan');
xLow=xM-nSD*std(X0);
...
% Drawing
F=figure('Position',[100,100,250,250],'Visible','off');
ax=gca;
xlabel(lab.xlab);
ylabel(lab.ylab);
hold on
...
% -density scatter:
scatter(xR,yR,7,'filled','MarkerEdgeColor','none','MarkerFaceColor',[0.5 0.5 0.5],'MarkerFaceAlpha',0.5);
...
[rho,pval] = corr(xR,yR,'type','Pearson');
...
% annotation('textbox',dim_right,'String',str_right,'Interpreter','tex','FontSize',8,'EdgeColor','none');
annotation('textbox',dim_right,'String',med,'Interpreter','tex','FontSize',18,'FontName','Arial','FontWeight','bold','EdgeColor','none');
