...
function [UIT]=drawFigDensRescaleV4met95CI(edgesX,X0,Y0,nSD,svPath,lgdLabel,med,lab,thrPct,scaleX,scaleY)
%% preparing data:
% Rescaling
xM=mean(X0,'omitnan');
yM=mean(Y0,'omitnan');
...
F=figure('Position',[100,100,350,410],'Visible','off');%can not change this
ax=gca;
hold on

%%%Option Three: contour
% ZZdensityContourF(xAll,yAll,colormap(flipud(gray)),ax,scaleX,scaleY);
scatter(xR.*scaleX/2,yR.*scaleY/2,5,'filled');
...
plot(t,yq,'-b','LineWidth',1);
patch([t fliplr(t)], [ylow fliplr(yupp)], 'b','EdgeColor','white')
alpha(0.3);
...
N=length(xAll);
set(ax, 'Position',[0.15, 0.08, 0.75, 0.65]);
