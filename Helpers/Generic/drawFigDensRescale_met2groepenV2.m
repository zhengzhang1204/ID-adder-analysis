...
function [slope1,slope2,N1,N2]=drawFigDensRescale_met2groepenV2(edgesX,X0,Y0,gp,nSD,svPath,lgdLabel,med,lab,thrPct)
% Rescaling
xM=mean(X0,'omitnan');
yM=mean(Y0,'omitnan');
xLow=xM-nSD*std(X0);
...
% -density scatter:
scatter(xR1,yR1,7,'filled','MarkerEdgeColor','none','MarkerFaceColor',[0 0.4470 0.7410],'MarkerFaceAlpha',0.3);

% DensScat(xR1,yR1,'TargetAxes',ax,'ColorBar',false,'ColorMap',flipud(gray));%,'ColorMap'
...
t=0.5:0.5:1.5;
yq1=b1(2)*t+b1(1);
plot(t,yq1,'-','LineWidth',1.5,'Color',[0 0.4470 0.7410]);
...
% -bin value1:
errorbar(value1,scatY1,SEM1,'Color',[0 0.4470 0.7410],'Marker','o','MarkerFaceColor','none','MarkerSize',5,'CapSize',0,'LineStyle','none');
axis([0 2 0 2])
ax=gca;
ax.FontSize=12;
xticks([0 1 2]);
...
scatter(xR2,yR2,7,'filled','MarkerEdgeColor','none','MarkerFaceColor',[0.8500 0.3250 0.0980],'MarkerFaceAlpha',0.3);
...
t=0.5:0.5:1.5;
yq2=b2(2)*t+b2(1);
plot(t,yq2,'-','LineWidth',1.5,'Color',[0.8500 0.3250 0.0980]);
...
% -bin value1:
errorbar(value2,scatY2,SEM2,'Color',[0.8500, 0.3250, 0.0980],'Marker','o','MarkerFaceColor','none','MarkerSize',5,'CapSize',0,'LineStyle','none');
hold off
...
str_left = {[lgdLabel,'_0 = ',num2str(slope1,'%0.2f')],...
    [lgdLabel,'_1 = ',num2str(slope2,'%0.2f')]};
str_right = {['N_0 = ',num2str(N1)],...
    ['N_1 = ',num2str(N2)]};


set(ax, 'Position',[0.15, 0.15, 0.75, 0.53]);
