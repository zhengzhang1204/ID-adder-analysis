% This function is to calculate the mean NCR in a time window ('win') and extract the distance from the window center to the closest initiatio and division.
function [uit]=outputtingK_cent(PHMap,PHLines,CIDinit,NCRmat,win)%In: PHMap(nx3),PHLines,SLines,NCRmat. Out:[NCR, T2nI, T2nD]
...
for iPHL=1:length(PHLines)%for each PHLines
    ...
    for j=1:size(PHLines{iPHL,1},1)%for each CID in current PHLine
        if j<=size(PHLines{iPHL,1},1)-win%the window is completely inside the current PHLine
            ...
            idx1=find(PHLinit{iPHL,1}>j+floor(win/2),1);%was:j+floor(win/2)-1
            if ~isempty(idx1)
                curT2nI=PHLinit{iPHL,1}(idx1)-j-floor(win/2);
            else
                if ~isnan(PHMap(iPHL,2)) && ~isempty(PHLinit{PHMap(iPHL,2),1})
                    idx1u=PHLinit{PHMap(iPHL,2),1}(1)+size(PHLines{iPHL,1},1)-j-floor(win/2);
                else
                    idx1u=[];
                end
                if ~isnan(PHMap(iPHL,3)) && ~isempty(PHLinit{PHMap(iPHL,3),1})
                    idx1l=PHLinit{PHMap(iPHL,3),1}(1)+size(PHLines{iPHL,1},1)-j-floor(win/2);
                else
                    idx1l=[];
                end
            ...
            if ~isnan(PHMap(iPHL,2)) 
                ...
                if size(PHLines{PHMap(iPHL,2),1},1)>=(win-size(CIDsP1,1)+1)
                    ...
                    if isnan(PHMap(PHMap(iPHL,2),2)) && isnan(PHMap(PHMap(iPHL,2),3))
                        ...
                        curT2nDU=size(PHLines{PHMap(iPHL,2),1},1)+size(PHLines{iPHL,1},1)-j-floor(win/2);
                    end
                end
                if ~isempty(PHLinit{PHMap(iPHL,2),1}) && isempty(curT2nI)
                    curT2nIu=PHLinit{PHMap(iPHL,2),1}(1)+size(PHLines{iPHL,1},1)-j-floor(win/2);
                else
                    curT2nIu=[];
                end
            end
            ...
            if ~isnan(PHMap(iPHL,3)) 
                ...
                if size(PHLines{PHMap(iPHL,3),1},1)>(win-size(CIDsP1,1)+1)
                    ...
                    if isnan(PHMap(PHMap(iPHL,3),2)) && isnan(PHMap(PHMap(iPHL,3),3))
                        ...
                        curT2nDL=size(PHLines{PHMap(iPHL,3),1},1)+size(PHLines{iPHL,1},1)-j-floor(win/2);
                    end
                end
                if ~isempty(PHLinit{PHMap(iPHL,3),1}) && isempty(curT2nI)
                    curT2nIl=PHLinit{PHMap(iPHL,3),1}(1)+size(PHLines{iPHL,1},1)-j-floor(win/2);
                else
                    curT2nIl=[];
                end
            end
        ...
        curT2nD=size(PHLines{iPHL,1},1)-j-floor(win/2);%was:-j-floor(win/2)+1
